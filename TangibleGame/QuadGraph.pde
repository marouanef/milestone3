class QuadGraph {
  ...
  boolean squareTest1(PVector c1, PVector c2, PVector c3, PVector c4) {
    float dist1 = c1.dist(c3);
    float dist2 = c2.dist(c4);
    float mean = (dist1 + dist2) / 2;
    return (mean * 0.8 < dist1 && mean * 1.2 > dist1) && (mean * 0.8 < dist1 && mean * 1.2 > dist1);
  }

  boolean squareTest2(PVector c1, PVector c2, PVector c3, PVector c4) {
    float dist1 = c1.dist(c2);
    float dist2 = c2.dist(c3);
    float dist3 = c3.dist(c4);
    float dist4 = c4.dist(c1);
    return abs(dist1 - dist3) < 0.9 * (dist1 + dist3) / 2 && abs(dist2 - dist4) < 0.9 * (dist2 + dist4) / 2;
  }
